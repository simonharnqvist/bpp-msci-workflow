...
rule generate_coding_bed:
	...
	conda:
		"env.yaml"
	shell:
		"""awk '$3="CDS"' {input} | awk '{{print $1, $4, $5}}' | awk '!visited[$0]++' | sed '/^#/d' > {output} """
...
rule select_random_regions:
	...
	shell:
		"""
		bedtools makewindows -b {input[0]} | awk '{{if($3-$2 <= 1000 && $3-$2 >= 500) print}}'| shuf | head -n {params.n_loci} > {output[0]}
		bedtools makewindows -b {input[1]} | awk '{{if($3-$2 <= 1000 && $3-$2 >= 500) print}}'| shuf | head -n {params.n_loci} > {output[1]}
		"""	
...
# This one should be several rules, but checkpointing is a nightmare; make standalone script
rule make_phylip:
	input:
